...
import { Button, Form, Select, DatePicker, Row, Col, Input, Radio, Divider, InputNumber, Tooltip, message } from 'antd'
...
class AddActivity extends Component {
    ...
    preview = () => {
      ...
      this.formRef.current.getFieldValue('time') === undefined || !poster
          ...
          data.description = this.activityEditor.current.getEditorValue() === '<p></p>' ? description : this.activityEditor.current.getEditorValue()   // 这里不知道什么原因
          data.poster = poster
        ...
        if(!values['companyId']) {
            ...
            this.props.dispatch({
                ...
                if(rsp.code === 0) {
                    ...
                    this.props.history.push('/activity/activityList/')
                }
            })
        }
    }
    ...
    render() {
        ...
        const { companyList } = this.props[namespace]
        ...
        return (
            <div className={Style.box}>
                <img className={Style.tag} alt="tag" src={require('../../assets/tag.png')}/>
                <div className={Style.tagName}>基本信息</div>
                <Form ref={this.formRef} name="control-ref" onFinish={this.onFinish}>
                <Row>
                <Col span={11}>
                  <Form.Item {...twoLayout} name="companyId" label="关联企业" rules={[ { required: true, message: '请选择企业'  } ]}>
                  <Select onChange={this.changeCompany}  placeholder="请选择企业" style={{marginLeft: '10px'}}>
                      {companyOptions}
                  </Select>
                  </Form.Item> 
                </Col>
                ...
                <Col span={24}>
                  <div className={Style.img}>
                     ...
                     <div style={{marginLeft: '-70px'}}>
                       <MyUpload imgUrl={poster} fn={this.uploadPoster} width={510} height={280} sign={1}/>
                     </div>
                ...
                <Col span={24}>
                  <Form.Item {...layout} name="comment" label="活动说明"  rules={[{required: true, message: '请填写活动地址'}]}>
                    <TextArea placeholder={activityType === 1 ? '请填写线上活动地址' : '请填写线下活动地址'} autoSize={{minRows: 2, maxRows: 3 }}/>
                  </Form.Item>
                </Col> 
                <Col span={11}>
